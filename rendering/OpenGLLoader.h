...
static draw_details upload_mesh(const std::vector<vertex>& verts, const std::vector<uint32_t> elem)
{
	...
	glBufferData(GL_ARRAY_BUFFER, verts.size() * sizeof(vertex), &verts[0], GL_STATIC_DRAW); // or we can use verts.data()
